...
document.addEventListener('DOMContentLoaded', function() {
  ...
  navLinks.forEach(link => {
    ...
    // 处理绝对路径和相对路径
    if (href.startsWith('/')) {
      linkPath = href.substring(1); // 移除开头的斜杠
    ...
    // 获取当前页面路径
    let currentPathProcessed = currentPath;
    // 移除开头的斜杠
    if (currentPathProcessed.startsWith('/')) {
      currentPathProcessed = currentPathProcessed.substring(1);
    }
    
    // 比较链接路径和当前路径
    if (currentPathProcessed.includes(linkPath)) {
      link.classList.add('active');
    } else if (linkPath === 'index.html' && (currentPathProcessed === '' || currentPathProcessed === 'index.html')) {
      // 特别处理首页
      link.classList.add('active');
    }
  });
